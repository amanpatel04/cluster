...
const UserInfo = () => {
  ...
  useEffect(() => {
    makeGetRequest('/user/get', true).then((response) => {
      let total = 0;
    ...
    <>
      <div className="h-full m-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="bg-white p-4 rounded-lg shadow border">
          ...
          <p className="text-xl font-semibold">{`${parseInt(userDetails.sizeUsed/1000000)} MB`}</p>
          <div className="mt-2 w-full bg-gray-200 h-2 rounded-full overflow-hidden">
            <div
              className="bg-green-500 h-2 rounded-full"
              style={{ width: `${(userDetails.sizeUsed / userDetails.sizeAllocated) * 100}%` }}
            ></div>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow border">
          <h2 className="text-sm text-gray-500">Total Capacity</h2>
          <p className="text-xl font-semibold">{userDetails.sizeAllocated/1e9} GB</p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow border">
          ...
          <p className="text-sm text-gray-400 mt-1">
            {`Last login: ${new Date(userDetails.updatedAt).toLocaleString()}`}
          </p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow border">
          <h2 className="text-sm text-gray-500">Total Files</h2>
          <p className="text-lg font-semibold text-green-600">{totalFiles}</p>
